...
**Candidate metrics to decide on**: per-file word count, capped well below 1,500; longest single bullet; non-negotiable count and density; frontmatter description length; bullets per file and per section; cross-file duplication of the same rule; and whether a file inline-enumerates another file's contents, which drifts by construction.

**Found instance, pending decision** (2026-07-28): `CLAUDE.md` and `skills/how-we-work/SKILL.md` both enumerate `shared/collaboration-workflow.md`'s bullets — "clarifying questions, propose-before-writing, commit-per-item, propose-improvements-to-shared-instructions, end-of-session review" — but that file now has seven bullets and both lists name the same five. "Make every content change visible" was already absent; the measurement bullet added 2026-07-28 made it two omissions. Nothing contradicts, but both lists need updating every time the source file changes, so they drift by construction — this is the enumeration metric above, caught by hand. Decide whether to sync both lists or reword so neither enumerates. Fixable on its own, ahead of the rest of this topic.

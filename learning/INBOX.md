...
- **The superseded-text convention is contested by the repo that used it** — `shared/persistent-docs.md:22` keeps stale wording in place under a `[Superseded YYYY-MM-DD: …]` marker. bbmon rejected that on 2026-08-05: the annotated `requirements.md` left a reader holding two contradictory statements and working out which was current, and git already provides the recovery the marker protects. It now deletes the stale text and puts the reasoning in the append-only log. Recorded as a deliberate divergence in `bbmon/docs/phase-1/log.md:50` and in that repo's memory, both saying "not yet proposed back" — and it never was, so the shared convention has stood unchallenged for eleven days while the only repo applying it did the opposite. Decide whether the convention changes, gains an opt-out, or stands with bbmon as a documented exception; route it through `propose-shared-change` so the target file's existing content gets checked. Second instance of a proposal parked in per-project memory — see the retrospective-trigger bullet above.

...
- **`onboard-project` has reached the file-size cap** — 794 prose words against `file_max_words` 800 as of 2026-08-05, after the `check-secret-scanning` pointer landed. The next addition of any size forces a split rather than a trim, per the hygiene doc's "split or promote, don't compress". Candidate seam: step 5 (reconciling a project's existing instruction files into project-specific deltas) is substantial and self-contained enough to stand as its own skill, leaving `onboard-project` as pure wiring.

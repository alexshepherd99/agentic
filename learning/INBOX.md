...
- Revisit `INBOX.md`'s own conventions — the question stands even though the pressure has passed, because it recurs on every handoff. The file has outgrown two of them: its title ("topics to explore") doesn't cover drafted, ready-to-apply edits, and while the three `fastf1_v1` proposals were parked here it pushed past `CLAUDE.md`'s ~1,500-word sprawl cap purely on entries designed to be deleted as soon as they land (they landed 2026-07-28, taking the file back to ~850). A word cap on a queue measures throughput, not sprawl — decide whether the cap should exclude transient sections, or whether long drafted text belongs somewhere else entirely (say `proposals/`, one file per proposal) with the INBOX holding only a pointer.

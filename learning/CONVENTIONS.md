...
- **Keep meta out of the instruction file.** The instruction file is loaded into context at runtime, so anything not needed to *perform* the skill is pure noise there — source links, influences, and design rationale belong in the skill-local `README.md` (the concrete rule lives in `skills/README.md` / `agents/README.md`).

...
Four defects have no threshold — any occurrence is a finding: the same rule stated in two files; a file inline-enumerating another file's contents; a reference to a file or section that doesn't exist; and a runtime-loaded file directing a read of one that isn't (`learning/CONVENTIONS.md`).
...
- **Duplication and enumeration flags are candidates, not verdicts.** Repeated *citation* phrasing across files is the consistency convention working correctly. Only repeated *rule content* is a defect, and separating the two needs a human.
